...
func (r *ExerciseRepository) CreateExercise(ctx context.Context, ex exercise.Exercise) (exercise.Exercise, error) {
	if ex.Name == "" || ex.Sets == 0 || ex.Reps == 0 || ex.Weight == 0 || ex.Description == "" {
		return exercise.Exercise{}, errors.New("invalid parameters")
	}

	var created exercise.Exercise
	query := `
		INSERT INTO exercises (name, sets, reps, weight, description)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, name, sets, reps, weight, description, created_at, updated_at
	`
	err := r.Pool.QueryRow(ctx, query, ex.Name, ex.Sets, ex.Reps, ex.Weight, ex.Description).Scan(
...
func (r *ExerciseRepository) UpdateExercise(ctx context.Context, ex exercise.Exercise) (exercise.Exercise, error) {
	...
	query := `
		UPDATE exercises
		SET name=$1, sets=$2, reps=$3, weight=$4, description=$5, updated_at=NOW()
		WHERE id=$7
		RETURNING id, name, sets, reps, weight, description, created_at, updated_at
	`
	err := r.Pool.QueryRow(ctx, query, ex.Name, ex.Sets, ex.Reps, ex.Weight, ex.Description, ex.ID).Scan(
...
func (r *ExerciseRepository) GetAllExercises(ctx context.Context) ([]exercise.Exercise, error) {
	query := `
		SELECT id, name, sets, reps, weight, description, created_at, updated_at
		FROM exercises
	`
	...
	for rows.Next() {
		...
		err := rows.Scan(&ex.ID, &ex.Name, &ex.Sets, &ex.Reps, &ex.Weight, &ex.Description, &ex.CreatedAt, &ex.UpdatedAt)
		if err != nil {
